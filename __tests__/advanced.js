...
describe("advanced mongodb features", () => {
    ...
    test("geoNear", async () => {
        ...
        const taxisNearbyOneKm = await Taxi.aggregate([
            {
                $geoNear: {
                    near: {
                        type: "Point",
                        coordinates: ClientNearbyNYCoordinates,
                    },
                    spherical: true,
        ...
        const taxisNearbyFiveThousandKm = await Taxi.aggregate([
            {
                $geoNear: {
                    near: {
                        type: "Point",
                    ...
                    spherical: true,
                    maxDistance: 5000 * 1000, //1 km
                    distanceField: "dist.calculated",
                },
            },
        ]);

        expect(taxisNearbyFiveThousandKm.length).toBe(2);
    });
});
